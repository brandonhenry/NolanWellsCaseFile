  ...
  function videoCard(item, group) {
    ...
    return `<article class="video-card${featured}" data-video-record data-video-category="${escapeHtml(item.category)}" data-video-search="${escapeHtml([item.title, item.publisher, item.date, item.description, item.editNote].join(' ').toLowerCase())}">
      <div class="video-stage" data-video-stage>
        <button class="video-load" type="button" data-video-src="${escapeHtml(item.file)}" data-video-poster="${escapeHtml(item.poster)}" data-video-title="${escapeHtml(item.title)}" aria-label="Load video: ${escapeHtml(item.title)}">
      ...
      <div class="video-card-body">
        <div class="video-meta"><span>${group === 'edited' ? 'Edited evidence reel' : 'Preserved excerpt'}</span><time>${escapeHtml(item.duration)}</time></div>
